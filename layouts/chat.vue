<template>
  <div id="chat-main">
    <main class="container">
      <aside class="servers">
        <div class="servers-collection">
          <div 
            ...
            <div class="server-icon"><svg><use xlink:href="#icon-friends" /></svg>
            </div>
            <div class="server-friends-online">{{ getNumberOfUserOnline }} Online</div>
          </div>
        </div>
      </aside>
      ...
      <aside class="channels">
        <header class="channels-header focusable">
          <h3 
            role="header" 
            class="channels-header-name">Lol Clan</h3>
          <svg 
            role="button" 
            aria-label="Dropdown" 
            class="channels-header-dropdown"><use xlink:href="#icon-dropdown" /></svg>
        </header>
        ...
        <section class="channels-list">
          ...
          <ul class="channels-list-text">
            <li class="channel focusable channel-text active">
              <span class="channel-name">lol</span>
              <button 
                class="button" 
                role="button" 
                aria-label="Invite"><svg><use xlink:href="#icon-invite" /></svg></button>
              <button 
                class="button" 
                role="button" 
                aria-label="settings"><svg><use xlink:href="#icon-channel-settings" /></svg></button>
            </li>
          </ul>
        </section>
        ...
        <footer class="channels-footer">
          ...
          <div class="channels-footer-details">
            ...
            <span class="tag">#{{ currenUserId }}</span>
          </div>
        </footer>
      </aside>
      ...
      <user-list :users="getLoggedInUsers"/>
    </main>

    <!-- ICONS -->
    <svg 
      id="icon-friends" 
      viewBox="-289 382 32 27.1"><style id="style3">.st0{fill:#FFFFFF;} .st1{opacity:0.6;}</style><g 
        id="g4145" 
        fill="#fff"><path 
          id="path5" 
    ...
    <svg 
      id="icon-dropdown" 
      viewBox="0 0 18 18"><style>.dd{stroke:#ABADAF;stroke-width:2px;stroke-dashoffset:1;stroke-dasharray:inherit}</style><path 
        class="dd" 
        stroke="#FFF" 
        d="M4.5 4.5l9 9" 
...
export default {
  ...
  computed: {
    ...
    getNumberOfUserOnline() {
      return this.$store.getters.getNumberOfUserOnline
    },
    getLoggedInUsers() {
      return this.$store.getters.getLoggedInUsers
    }
  }
}
</script>
